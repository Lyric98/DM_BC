...
def crop_to_brightest_region(img, target_size):
    """Crops the image to a square centered on the brightest region, then resizes to the target size."""
    img_array = np.array(img)
    img_height, img_width = img_array.shape

    # Calculate brightness sum for each row along the full width
    row_sums = np.sum(img_array, axis=1)

    # Find the row with the maximum brightness sum
    brightest_row = np.argmax(row_sums)

    # Define the crop boundaries centered on the brightest row
    half_crop = img_width // 2
    left = 0
    right = img_width
    upper = max(brightest_row - half_crop, 0)
    lower = min(brightest_row + half_crop, img_height)

    # Ensure the crop is exactly square by adjusting bounds if necessary
    if lower - upper < img_width:
        if upper == 0:
            lower = img_width
        elif lower == img_height:
            upper = img_height - img_width
        else:
            upper = max(brightest_row - half_crop, 0)
            lower = upper + img_width
...
# def resize_and_convert(img, size, resample):
#     """Resize and crop using the brightest-centered cropping function."""
#     if img.size[0] != size:
#         img = crop_to_brightest_region_along_height(img, size)
#         img = img.resize((size, size), resample=resample)

#     # Convert to RGB (3 channels) after processing
#     img = img.convert('RGB')
#     return img

def resize_and_convert(img, size, resample):
    """Resize and crop using the brightest-centered cropping function, then convert to RGB."""
    # Convert to grayscale for brightest-centered cropping
    img_gray = img.convert('L')  # Convert to grayscale
    img_cropped = crop_to_brightest_region(img_gray, size)
    
    # Resize the cropped image to the desired size
    img_cropped = img_cropped.resize((size, size), resample=resample)
...
def resize_worker(img_file, sizes, resample, lmdb_save=False):
    """Process each image file with resizing and brightest-centered cropping."""
...
def prepare_process_worker(wctx, file_subset):
    for file in file_subset:
        ...
        if not wctx.lmdb_save:
            lr_img.save(
                f"{wctx.out_path}/lr_{wctx.sizes[0]}/{i.zfill(5)}.png")
            hr_img.save(
                f"{wctx.out_path}/hr_{wctx.sizes[1]}/{i.zfill(5)}.png")
            sr_img.save(
                f"{wctx.out_path}/sr_{wctx.sizes[0]}_{wctx.sizes[1]}/{i.zfill(5)}.png")
        else:
            with wctx.env.begin(write=True) as txn:
                txn.put(f'lr_{wctx.sizes[0]}_{i.zfill(5)}'.encode('utf-8'), lr_img)
                txn.put(f'hr_{wctx.sizes[1]}_{i.zfill(5)}'.encode('utf-8'), hr_img)
                txn.put(f'sr_{wctx.sizes[0]}_{wctx.sizes[1]}_{i.zfill(5)}'.encode('utf-8'), sr_img)
...
def all_threads_inactive(worker_threads):
    return not any(thread.is_alive() for thread in worker_threads)
...
def prepare(img_path, out_path, n_worker, sizes=(16, 128), resample=Image.BICUBIC, lmdb_save=False):
    ...
    if n_worker > 1:
        ...
        for subset in file_subsets:
            proc = Process(target=prepare_process_worker, args=(wctx, subset))
            proc.start()
            worker_threads.append(proc)
        ...
        prepare_process_worker(wctx, files)
